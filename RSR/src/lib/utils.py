...
class Data(object):
    ...
    def apply_model(self, model: Model, ctx_dic=None):
        ...
        with tracer.start_as_current_span("apply_model_util",context=ctx) as amu:
            context = amu.get_span_context()
            context_dict = {'traceId': context.trace_id,
                 'spanId': context.span_id}
            if len(self.model_results) == 0:  # Apply the model if not already
                for w in self.workers:
                    self.model_results.append(w.apply_model.remote(model, context_dict))
                print(ray.get(self.model_results))
            pass

    def collect_data(self, convertor:Convertor=None, context_dict=None):
        tracer = trace.get_tracer(__name__)
        ctx = get_parent_context(context_dict["traceId"], context_dict["spanId"])

        with tracer.start_as_current_span("collect_data", context=ctx) as cd:
            context = cd.get_span_context()
            context_dict = {'traceId': context.trace_id,
                'spanId': context.span_id}

            output_map_futures = []
            for w in self.workers:
                output_map_futures.append(w.get_output_files.remote(convertor, context_dict))
...
class Archive:
    ...
    """
    def __init__(self, cluster_config: ClusterConfig, context_dict=None):
...
class GoogleArchive(Archive):
    ...
    """
    def __init__(self, cluster_config: ClusterConfig, context_dict):
        Archive.__init__(self, cluster_config, context_dict)
        self.unique_urls = []
        self.true_urls_per_worker = []
        self.tile_mapping = collections.defaultdict(dict)

    # TODO: Trace this function
    def get_urls_for_tile(self, data, context_dict=None):
        """
        Fetch urls for downloading data
        Parameters: path, row, bands
        """
        ctx = get_parent_context(self.context_dict["traceId"], self.context_dict["spanId"])
        tracer = trace.get_tracer(__name__)
        
        with tracer.start_as_current_span("get_urls_for_tile", context=ctx) as fu:
            ########################################################################################################
    ...
    def fetch_urls(self, limit=None, parallel=False, per_tile=False):
        ...
        with tracer.start_as_current_span("fetch_urls", context=ctx) as fu:
            context = fu.get_span_context()
            context_dict = {'traceId': context.trace_id,
                'spanId': context.span_id}
            self.unique_urls = []
            if parallel:
                ...
                for path in range(self.bound_box.get_start_path(), self.bound_box.get_end_path()-1, -1):
                    for row in range(self.bound_box.get_start_row(), self.bound_box.get_end_row()+1):
                        print("Getting urls for Path: {} Row: {}".format(path, row))
                        self.unique_urls.extend(self.get_urls_for_tile([path, row], context_dict))
            if limit:
    ...
    def redistribute(self, compression=False) -> Data:
        ...
        with tracer.start_as_current_span("garch_redistribute", context=ctx) as gr:
            ...
            context = gr.get_span_context()
            context_dict = {'traceId': context.trace_id,
                 'spanId': context.span_id}

            for worker in self.cluster_config.get_workers():
                res.append(worker.redistribute.remote(compression, context_dict))
            futures = ray.get(res)
            end_time = clock.time()
    ...
    def download(self) -> Data:
        ...
        with tracer.start_as_current_span("download", context=ctx) as d:
            context = d.get_span_context()
            context_dict = {'traceId': context.trace_id,
                'spanId': context.span_id}
            ...
            for i in range(self.cluster_config.get_num_workers()):
                ...
                res.append(self.cluster_config.get_workers()[i].pull_data.remote(
                    self.unique_urls[start:start + true_url_per_worker[i]], context_dict))
                start += true_url_per_worker[i]
            ray.get(res)
            end_time = clock.time()
